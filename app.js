...
document.querySelectorAll(".tab").forEach(b=>b.onclick=()=>{
 ...
 $("#addressPane").classList.toggle("hidden",b.dataset.tab!=="address");
 $("#codesPane").classList.toggle("hidden",b.dataset.tab!=="codes");
});

async function api(path, opts={}){
 if(!worker()||worker().includes("YOUR-WORKER"))throw new Error("config.js에 Cloudflare Worker 주소를 먼저 입력하세요.");
...
async function health(){
 try{const j=await api("/api/health");$("#apiState").textContent=j.keyConfigured?"API 연결됨":"API 키 미설정";$("#apiState").style.background=j.keyConfigured?"#027a48":"#b54708"}catch(e){$("#apiState").textContent="Worker 연결 안 됨"}
}
health();

$("#searchAddress").onclick=async()=>{
 const address=$("#address").value.trim();if(!address)return setStatus("주소를 입력하세요.","error");
 setStatus("주소를 분석하고 건축물대장을 조회하고 있습니다...");
 try{
...
$("#searchCodes").onclick=()=>runQuery({
 sigunguCd:$("#sigunguCd").value.trim(),bjdongCd:$("#bjdongCd").value.trim(),
 platGbCd:$("#platGbCd").value,bun:pad4($("#bun").value),ji:pad4($("#ji").value)
},"코드 직접 조회");

async function runQuery(q,label){
 if(!/^\d{5}$/.test(q.sigunguCd||"")||!/^\d{5}$/.test(q.bjdongCd||""))return setStatus("시군구코드와 법정동코드는 각각 5자리여야 합니다.","error");
 setStatus("표제부 · 층별 · 전유부 데이터를 불러오고 있습니다...");
 try{
...
function render(d,label){
 ...
 $("#buildingTitle").textContent=t.platPlc||t.newPlatPlc||label||"";
 const park=n(t.totPkngCnt)||n(t.indrAutoUtcnt)+n(t.oudrAutoUtcnt)+n(t.indrMechUtcnt)+n(t.oudrMechUtcnt);
 $("#summaryCards").innerHTML=[
 ["주용도",t.mainPurpsCdNm||t.etcPurps||"-"],["연면적",t.totArea?fmt(t.totArea)+"㎡":"-"],
 ["지상/지하",`${t.grndFlrCnt||"-"} / ${t.ugrndFlrCnt||"-"}층`],["사용승인일",date8(t.useAprDay)],["주차",park?fmt(park,0)+"대":"-"]
 ...
 $("#basicInfo").innerHTML=infos.map(([a,b])=>`<div class="info"><span>${a}</span><b>${esc(b||"-")}</b></div>`).join("");
 table("#titlesTable",[
 ...
 $("#unitNotice").textContent=d.units.length?"※ 공용면적 구분은 건축물대장에 기재된 용도 문자열을 기준으로 분류합니다.":"전유부/전유공용면적 데이터가 없는 일반 건축물이거나 해당 대장에 호별 정보가 없습니다.";
}
function table(sel,cols,rows){
 const e=$(sel);e.dataset.cols=JSON.stringify(cols.map(c=>c[0]));
 e.innerHTML="<thead><tr>"+cols.map(c=>`<th>${c[0]}</th>`).join("")+"</tr></thead><tbody>"+
 rows.map(r=>"<tr>"+cols.map(([_,k])=>{let v=typeof k==="function"?k(r):r[k]; return `<td>${esc(v??"-")}</td>`}).join("")+"</tr>").join("")+"</tbody>";
}
function filterTable(input,table){
 $(input).addEventListener("input",e=>{const q=e.target.value.toLowerCase();$(table).querySelectorAll("tbody tr").forEach(tr=>tr.style.display=tr.textContent.toLowerCase().includes(q)?"":"none")})
}
filterTable("#floorFilter","#floorsTable");filterTable("#unitFilter","#unitsTable");

document.querySelectorAll("[data-csv]").forEach(b=>b.onclick=()=>{
 const key=b.dataset.csv, rows=DATA[key]||[]; if(!rows.length)return alert("내보낼 데이터가 없습니다.");
 let cols;
 ...
 const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv;charset=utf-8"}));a.download=`building-${key}.csv`;a.click();URL.revokeObjectURL(a.href);
});
